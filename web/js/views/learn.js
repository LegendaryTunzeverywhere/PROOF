    ...
    <div class="row" style="gap:10px;align-items:center">
      ...
      <div style="flex:1">
        <div class="row" style="gap:6px;align-items:baseline">
          <b style="font-size:15px;color:var(--ink)">${streak.currentStreak}-day streak</b>
          ${streak.atRisk ? `<span class="chip" style="background:var(--warn-soft);color:var(--warn);padding:2px 6px;font-size:9px;font-weight:800">AT RISK</span>` : ''}
        </div>
        <div class="tiny" style="color:var(--ink-2);margin-top:2px">${message}</div>
      </div>
      <div style="text-align:center;min-width:42px">
        <b class="num" style="font-size:20px;color:${barColor};display:block">${streak.currentStreak}</b>
        <span class="tiny" style="color:var(--muted);font-size:9px">DAYS</span>
      </div>
    </div>
  </div>`;
}
...
/**
 * Show modal to input learning goal
 * @param {Function} callback - Called with goal string or null if cancelled
 */
function showGoalModal(callback) {
  ...
  overlay.innerHTML = `
    <div class="modal-card">
      <div class="modal-header">
        <h2 class="modal-title">What do you want to learn?</h2>
        <button class="modal-close" id="modalClose">${ico.x.replace('<svg', '<svg width="20" height="20"')}</button>
      </div>
      <div class="modal-body">
        <input 
          ...
          autofocus
        />
        <div class="modal-suggestions" id="modalSuggestions">
          <span class="modal-suggestion-label">Quick picks:</span>
          ${SKILLS_CATALOG.slice(0, 6).map(([slug, emoji, name]) => 
            `<button class="chip chip-suggest" data-goal="I want to learn ${name.toLowerCase()}">${emoji} ${name}</button>`
          ).join('')}
        </div>
      </div>
      <div class="modal-footer">
        <button class="btn btn-ghost" id="modalCancel">Cancel</button>
        <button class="btn btn-primary" id="modalSubmit">Create Path</button>
      </div>
    </div>
  `;
  ...
  // Suggestion chips
  overlay.querySelectorAll('[data-goal]').forEach((chip) => {
    chip.addEventListener('click', () => {
      input.value = chip.dataset.goal;
      input.focus();
    });
  });
}
...
function pathCard(p) {
  return `<div class="card card-click" data-path="${p.id}" style="padding:18px">
    ...
    <div class="row mt12" style="gap:10px;align-items:center">
      <div class="bar" style="flex:1"><i style="width:${p.percent}%"></i></div><span class="tiny num" style="font-weight:800;color:var(--primary)">${p.percent}%</span>
    </div></div>`;
}
...
export async function pathScreen(root, { id }) {
  ...
  root.innerHTML = `<div class="pad" style="padding-top:max(14px, env(safe-area-inset-top))">
    ...
    <div class="card card-hero mt16" style="padding:20px">
      <div style="font-size:30px">${p.skillEmoji}</div>
      <h1 class="h1 mt8" style="color:#fff">${esc(p.title)}</h1>
      <p class="sub mt8" style="color:rgba(255,255,255,.72)">${esc(p.description)}</p>
      <div class="row mt12" style="gap:10px;align-items:center">
        <div class="bar" style="flex:1;background:rgba(255,255,255,.2)"><i style="width:${p.percent}%;background:var(--nim-grad)"></i></div>
        <b class="num" style="color:#fff;font-size:14px">${p.percent}%</b>
      </div>
      <div class="row mt12" style="gap:8px;flex-wrap:wrap">
        <span class="chip chip-dark">${p.level}</span><span class="chip chip-dark">${p.minutesPerDay} min/day</span>
        <span class="chip chip-dark">${p.totalXp} XP total</span>
      </div>
    </div>

    <div class="section bento-full"><div class="section-head"><span class="eyebrow">${p.days.length}-DAY SKILL PATH</span><span class="tiny">tap a day</span></div>
      <div>${p.days.map((d) => dayBlock(p, d)).join('')}</div></div>
  </div>`;
  ...
  // auto-open the current day
  const current = p.days.find((d) => d.items.some((i) => !i.lessonDone && !i.practiceDone && !i.attempt));
  if (current) $(`.day[data-index="${current.index}"]`, root)?.classList.add('open');
  else $('.day', root)?.classList.add('open');
}
...
function dayBlock(p, d) {
  ...
  return `<div class="day ${d.kind !== 'study' ? 'proof-day' : ''} ${allDone ? 'done' : ''}" data-index="${d.index}">
    <div class="day-head">
      <div class="day-num">${d.kind !== 'study' ? icon : d.index}</div>
      <div style="flex:1">
        <b style="font-size:14.5px">Day ${d.index} · ${esc(d.title)}</b>
        <div class="tiny">${d.estMin} min · +${d.xp} XP ${d.rewardNim ? `· <span style="color:var(--nim-deep);font-weight:800">+${d.rewardNim} NIM</span>` : ''}</div>
      </div>
      ${allDone ? '<span class="chip chip-ok" style="padding:4px 10px">✓</span>' : `<span class="tiny">${d.kind !== 'study' ? 'PROOF' : ''}</span>`}
    </div>
    <div class="day-body">
      ${d.items.map((i) => itemRow(d, i)).join('')}
    </div>
  </div>`;
}

function itemRow(day, i) {
  const status = i.attempt
    ? `<span class="chip ${i.attempt.status === 'passed' ? 'chip-ok' : 'chip-bad'}" style="padding:4px 10px">${i.attempt.status === 'passed' ? '✓' : '·'} ${i.attempt.score}</span>`
    : i.practiceDone ? '<span class="chip chip-primary" style="padding:4px 10px">✓</span>' : '';
  return `<div class="row" style="padding:10px 4px;border-top:1px solid var(--line)">
    <span style="font-size:17px">${i.kind === 'study' ? (i.practiceDone ? '✏️' : '📖') : i.kind === 'final' ? '🏆' : i.kind === 'project' ? '🛠️' : '🎯'}</span>
    <div style="flex:1">
      <b style="font-size:13.5px">${esc(i.title)}</b>
      <div class="tiny">${i.kind === 'study' ? 'Lesson + practice' : `${i.kind === 'final' ? 'Final assessment' : i.kind === 'project' ? 'Project proof' : 'Proof checkpoint'} · ${i.estMin} min ${i.rewardNim ? `· ${i.rewardNim} NIM` : ''}`}</div>
    </div>
    ${status}
    ${i.kind === 'study'
      ? `<button class="btn btn-soft btn-sm" data-lesson="${i.topic}" data-day="${day.index}">${i.lessonDone ? 'Review' : 'Learn'}</button>`
      : `<button class="btn ${i.attempt?.status === 'passed' ? 'btn-ghost' : 'btn-nim'} btn-sm" data-challenge="${i.challengeId}">${i.attempt ? (i.attempt.status === 'passed' ? 'Review' : 'Retry') : 'Start proof'}</button>`}
  </div>`;
}
...
export async function lessonScreen(root, { pathId, topic }) {
  ...
  // First fetch the path to get skillSlug
  const pathRes = await api.get(`/api/paths/${pathId}`);
  const p = pathRes.path;
  
  // Then fetch the lesson using the skillSlug from the path
  ...
  // Check if this is a first-time lesson (not lessonDone)
  // Offer pre-lesson grilling unless they've skipped it
  if (!item.lessonDone && !skipGrilling) {
    showPreLessonPrompt(root, pathId, topic, dayIndex, lesson, p);
    return;
  }
...
/**
 * Show pre-lesson grilling prompt
 */
function showPreLessonPrompt(root, pathId, topic, dayIndex, lesson, path) {
  root.innerHTML = `<div class="pad" style="padding-top:max(14px, env(safe-area-inset-top))">
    ...
    <div class="pre-lesson-card">
      <div class="pre-lesson-icon">🎯</div>
      <h2>Before we start...</h2>
      <p class="pre-lesson-intro">
        Let's spend 2-3 minutes on a few questions about 
        <strong>${esc(lesson.title)}</strong>.
      </p>
      <p class="pre-lesson-why">
        This isn't a test — it helps you learn deeper by grounding what you're about to learn in your own context and experience.
      </p>
      
      <div class="pre-lesson-benefits">
        <div class="benefit-item">
          <span class="benefit-icon">💡</span>
          <span>Surface what you already know</span>
        </div>
        <div class="benefit-item">
          <span class="benefit-icon">🎯</span>
        ...
        <div class="benefit-item">
          <span class="benefit-icon">🧠</span>
          <span>Make connections that stick</span>
        </div>
      </div>
      
      <div class="pre-lesson-actions">
        <button class="btn-primary btn-block" id="startGrilling">
          Start Pre-Lesson Questions
        </button>
        <button class="btn-ghost btn-block mt12" id="skipGrilling">
          Skip and go straight to lesson
        </button>
      </div>
    </div>
  </div>`;
  ...
  root.querySelector('#back').addEventListener('click', () => {
    location.hash = `#/learn/path/${pathId}`;
  });

  root.querySelector('#startGrilling').addEventListener('click', () => {
    startSocraticSession('pre_lesson', topic, lesson.title, {
      skillSlug: path.skillSlug,
      lessonUrl: `#/learn/lesson/${pathId}/${topic}?day=${dayIndex}&skip_grilling=1`
    });
  });

  root.querySelector('#skipGrilling').addEventListener('click', () => {
    location.hash = `#/learn/lesson/${pathId}/${topic}?day=${dayIndex}&skip_grilling=1`;
  });
}
...
/**
 * Start a checkpoint Socratic session
 */
function startCheckpointSession(checkpointNum, topicSlug, lessonTitle, skillSlug) {
  startSocraticSession('checkpoint', topicSlug, lessonTitle, {
    skillSlug,
    checkpointNumber: checkpointNum,
    checkpointContext: 'Mid-lesson understanding check'
  });
}
...
function runPractice(root, pathId, day, item, lesson) {
  ...
  function render() {
    const q = qs[idx];
    zone.innerHTML = `<div class="card mt12">
      <div class="row-between"><span class="eyebrow">PRACTICE ${idx + 1}/${qs.length}</span><span class="tiny">+10 XP when done</span></div>
      <b style="display:block;margin-top:10px;font-size:15.5px">${esc(q.q)}</b>
      <div class="mt12" id="opts">${q.choices.map((c, i) => `<button class="opt" data-i="${i}">${'ABCD'[i]}. ${esc(c)}</button>`).join('')}</div>
      <div id="verdict"></div>
    </div>`;
    $$('#opts .opt', zone).forEach((b) => b.addEventListener('click', () => {
      ...
      if (right) correct++;
      zone.querySelector('#verdict').innerHTML = `
        <div class="callout mt12" style="background:${right ? 'var(--ok-soft)' : 'var(--bad-soft)'};color:${right ? 'var(--ok-deep)' : 'var(--bad)'}">
          <b>${right ? 'Correct — ' : 'Not quite. '}</b>${esc(q.why || q.hint || '')}</div>
        <button class="btn btn-primary btn-block mt12" id="nextQ">${idx + 1 < qs.length ? 'Next question' : 'Finish practice'}</button>`;
      zone.querySelector('#nextQ').addEventListener('click', async () => {
        idx++;
        if (idx < qs.length) render();
        else {
          const r = await api.post(`/api/paths/${pathId}/progress`, { dayIndex: day.index, topicSlug: item.topic, part: 'practice' });
          
          // Show post-lesson reflection prompt
...
function showPostLessonReflection(zone, pathId, day, item, lesson, correctCount, totalCount) {
  ...
  zone.innerHTML = `<div class="card mt12">
    <div class="reflection-header">
      <div style="font-size:36px">${correctCount === totalCount ? '🌟' : '💪'}</div>
      <b>${correctCount}/${totalCount} correct</b>
    </div>
    ...
    <div class="reflection-prompt">
      ...
      <div class="reflection-content">
        ...
        <p>Before moving on, let's solidify what you learned.</p>
        <button class="btn-primary btn-block" id="startReflection">
          Start Reflection (2 min)
        </button>
        <button class="btn-ghost btn-block mt8" id="skipReflection">
          Skip reflection
        </button>
      </div>
    </div>
  ...
  zone.querySelector('#startReflection').addEventListener('click', () => {
    startSocraticSession('reflection', item.topic, lesson.title, {
      skillSlug: lesson.skillSlug || 'web-development',
      practiceScore: `${correctCount}/${totalCount}`,
      proofAhead: !!proofItem
    });
  });
  
  zone.querySelector('#skipReflection').addEventListener('click', () => {
    showProofOrComplete(zone, pathId, proofItem);
  });
}

/**
 * Show proof CTA or completion message
 */
function showProofOrComplete(zone, pathId, proofItem) {
  zone.innerHTML = `<div class="card mt12 center">
    <div style="font-size:36px">✅</div>
    <b>Lesson complete!</b>
    <p class="sub mt8">${proofItem ? 'You know the concepts. Now prove it for real.' : 'Concepts locked in. Keep the streak alive tomorrow.'}</p>
    ${proofItem ? `<button class="btn btn-nim btn-block mt8" id="goProof">${ico.bolt.replace('<svg', '<svg width="16" height="16"')} Start proof · ${proofItem.rewardNim ? `+${proofItem.rewardNim} NIM` : `+${proofItem.xp} XP`}</button>` : `<a href="#/learn/path/${pathId}" class="btn btn-soft btn-block mt8">Back to path</a>`}
  </div>`;
  
  zone.querySelector('#goProof')?.addEventListener('click', () => {
    location.hash = `#/prove/challenge/${proofItem.challengeId}`;
  });
}
  ...
  const s = sheet(`
    <div class="row-between" style="gap:10px">
      <div class="row" style="gap:10px">
        <div class="avatar av-32" style="background:var(--grad-hero);color:#fff">${ico.sparkle.replace('<svg', '<svg width="15" height="15"')}</div>
        <div><b>PROOF Tutor</b><div class="tiny">${esc(lesson.title)} · guides, never just answers</div></div>
      </div>
      <button class="btn btn-ghost btn-icon" id="closeChat" title="Close">${ico.x.replace('<svg', '<svg width="18" height="18"')}</button>
    </div>
    <div class="chat mt8" id="chatLog"><div class="msg ai">Hey — I'm your tutor for <b>${esc(lesson.title)}</b>. Ask me anything, or pick one:</div></div>
    <div class="chip-row" id="quick">
      <button class="chip" data-q="Explain it simpler">Explain simpler</button>
      <button class="chip" data-q="Show me an example">Example</button>
      <button class="chip" data-q="Give me an exercise">Exercise</button>
      <button class="chip" data-q="I'm stuck, give me a hint">Hint</button>
    </div>
    <div class="row mt8" style="gap:8px">
      <input class="input" id="chatInput" placeholder="Ask about this lesson…" style="flex:1"/>
      <button class="btn btn-primary" id="chatSend" style="padding:12px 15px">${ico.send.replace('<svg', '<svg width="17" height="17"')}</button>
    </div>`, { });
