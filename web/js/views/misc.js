...
export async function leaderboardScreen(root) {
  ...
  async function load(c) {
    ...
    root.querySelector('#lbBody').innerHTML = `<div class="stack">
      ${entries.map((e, i) => `
        <div class="card" style="padding:13px 16px">
          <div class="row" style="gap:12px">
            ...
            <div class="avatar av-32">${e.avatar}</div>
            <div style="flex:1;min-width:0">
              <b style="font-size:14px;display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">
                ${esc(e.username)}
                ${e.walletAddress && !e.isDemo ? `<span style="color:var(--primary);font-size:11px;margin-left:4px">✓</span>` : ''}
                ${e.isDemo ? `<span class="tiny" style="color:var(--muted);margin-left:4px">(demo)</span>` : ''}
              </b>
              ${e.walletAddress && !e.isDemo 
                ? `<div class="tiny" style="color:var(--primary);font-family:monospace;margin-top:2px">${e.walletAddress.substring(0, 20)}...</div>`
                : `<span class="tiny">Level ${e.level} · ⭐ ${e.reputation} · ${e.proofsPassed} proofs</span>`
              }
            </div>
            <b class="num" style="font-size:14px;color:var(--primary-deep)">${e.value}</b>
          </div></div>`).join('') || '<div class="card"><div class="empty">No entries yet.</div></div>'}
    </div>`;
  }
...
export async function notificationsScreen(root) {
  ...
  root.innerHTML = `<div class="pad bento-read" style="padding-top:max(14px, env(safe-area-inset-top))">
    <div class="row-between"><button class="btn btn-ghost btn-sm" onclick="history.back()">${ico.back} Back</button>
      <span class="eyebrow">🔔 NOTIFICATIONS</span>
      <button class="btn btn-soft btn-sm" id="readAll" ${unread ? '' : 'disabled'}>Mark read</button></div>
    <div id="walletStatusPlaceholder" class="mt8"></div>
    <div class="stack mt16">
      ${notifications.map((n) => `
        <div class="card ${n.read ? '' : ''}" style="padding:13px 16px;${n.read ? '' : 'border-left:3px solid var(--primary)'}">
          <div class="row" style="gap:12px">
            <span style="font-size:20px">${n.emoji}</span>
            <div style="flex:1"><b style="font-size:13.5px">${esc(n.title)}</b>
              ${n.body ? `<div class="tiny mt8" style="color:var(--muted)">${esc(n.body)}</div>` : ''}
              <span class="tiny">${timeAgo(n.createdAt)}</span></div>
          </div></div>`).join('') || '<div class="card"><div class="empty"><span class="big">🌙</span>All quiet. Go learn something.</div></div>'}
    </div></div>`;
...
export async function publicProfileScreen(root, { username }) {
  ...
  root.innerHTML = `<div class="pad bento-read" style="padding-top:max(14px, env(safe-area-inset-top))">
    ...
    <div class="card card-hero mt16" style="padding:22px 20px;text-align:center">
      ...
      <div class="row mt12" style="gap:8px;justify-content:center;flex-wrap:wrap">
        <span class="chip chip-dark">${profile.proofsCompleted} proofs passed</span>
        <span class="chip chip-dark">${profile.tasksAccepted} gigs</span>
        <span class="chip chip-dark">${profile.teachingSessions} sessions</span>
      </div>
    ...
    <div class="section"><div class="section-head"><span class="eyebrow">✓ VERIFIED SKILLS</span></div>
      <div class="stack">${profile.verifiedSkills.length ? profile.verifiedSkills.map((s) => `
        <div class="card" style="padding:14px 16px">
          <div class="row-between">
            <b style="font-size:14px;text-transform:capitalize">${esc(s.skillSlug.replace(/-/g, ' '))}</b>
            <span class="chip chip-ok">✓ ${s.score}% · ${esc(s.tier)}</span>
          </div>
          <div class="bar bar-ok mt8"><i style="width:${s.score}%"></i></div>
        </div>`).join('') : '<div class="card"><div class="empty" style="padding:16px">Nothing verified yet.</div></div>'}</div></div>

    ${profile.proofs.length ? `<div class="section"><div class="section-head"><span class="eyebrow">📜 PROOF HISTORY</span></div>
      <div class="stack">${profile.proofs.slice(0, 10).map((p) => `
        <div class="card" style="padding:12px 16px">
          <div class="row-between"><span style="font-size:13px;font-weight:650">${esc(p.challengeTitle)}</span>
            <span class="chip ${p.passed ? 'chip-ok' : 'chip-bad'}" style="padding:3px 9px;font-size:11px">${Math.round(p.score)}/100</span></div>
          <div class="tiny mt8" style="color:var(--muted)">${timeAgo(p.completedAt)}</div>
        </div>`).join('')}</div></div>` : ''}
